...
class TestBun:
    ...
    def test_get_name_negative(self, none=None):
        # Создаем объект класса и без параметров
        emptyBun = Bun(none, none)
        # Проверяем, что
        assert emptyBun.get_name() == None
    ...
    def test_get_price_negative(self, none=None):
        # Создаем объект класса и без параметров
        emptyBun = Bun(none, none)
        assert emptyBun.get_price() == None